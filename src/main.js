...
import { createRadioAutoplayManager } from "./shared/radio.js";

const rootElement = document.getElementById("app");

function App({ utils, catalogSource, sanityConfig }) {
  const [catalog, setCatalog] = React.useState(null);
  const radioManagerRef = React.useRef(null);
  ...
  React.useEffect(() => {
    const configuredVolume = Number(sanityConfig.radioVolume);

    radioManagerRef.current = createRadioAutoplayManager({
      volume: Number.isFinite(configuredVolume) ? configuredVolume : 1,
    });

    return () => {
      if (radioManagerRef.current) {
        radioManagerRef.current.destroy();
        radioManagerRef.current = null;
      }
    };
  }, [sanityConfig]);
  ...
  React.useEffect(() => {
    ...
    radioManagerRef.current.updateFromCatalog(catalog);
  }, [catalog]);
